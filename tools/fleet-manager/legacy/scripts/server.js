...
const { PORTS: RBK_PORTS } = require("../shared/rbk");
const { loadMapGraphLight } = require("../shared/map_loader");
...
const localSharedDir = path.join(__dirname, "shared");
const trafficCoreDir = path.resolve(__dirname, "..", "fleet-traffic-core", "src");
...
const algorithmsModulePath = path.resolve(__dirname, "..", "fleet-sim-core", "src", "algorithms.js");
const trafficStrategiesModulePath = path.resolve(
  __dirname,
  "..",
  "fleet-traffic-core",
  "src",
  "traffic_strategies.js"
);

let Algorithms = null;
let TrafficStrategies = null;
try {
  Algorithms = require(algorithmsModulePath);
} catch (_err) {
  // optional module
}
try {
  TrafficStrategies = require(trafficStrategiesModulePath);
} catch (_err) {
  // optional module
}
...
app.use(express.json({ limit: rdsMaxBodyLength }));
...
app.use("/shared", express.static(localSharedDir));
app.use("/traffic-core", express.static(trafficCoreDir));
...
roboshop.post("/uploadScene", async (req, res) => {
  try {
    ...
    let response = null;
    try {
      response = await rds.uploadScene(buffer);
    } catch (err) {
      response = { statusCode: scene ? 200 : 500, body: null, error: err };
    }

    const payload =
      response && response.body && typeof response.body === "object"
        ? Object.assign({}, response.body)
        : {
            code: response && response.statusCode >= 400 ? 40000 : 0,
            create_on: new Date().toISOString(),
            msg: response && response.statusCode >= 400 ? "upload_failed" : "ok"
          };
    if (savedPath) {
    ...
    if (response && response.error) {
      payload.upstream_error = response.error.message;
    }
    const statusCode = response.statusCode || (scene ? 200 : 500);
...
roboshop.post("/getCoreParam", async (req, res) => {
  try {
    ...
    const body = parsed.body || { param: "", plugin: "" };
    const response = await rds.getCoreParam(body);
    return sendJson(res, response.statusCode || 200, response.body || {});
  } catch (err) {
    return sendJson(res, 500, { error: "proxy_failed", message: err.message });
  }
...
roboshop.post("/saveCoreParam", async (req, res) => {
  try {
    ...
    const body = parsed.body || {};
    const response = await rds.saveCoreParam(body);
    const contentType = response.headers && response.headers["content-type"];
    return sendText(res, response.statusCode || 200, response.body || "", contentType);
  } catch (err) {
    return sendJson(res, 500, { error: "proxy_failed", message: err.message });
  }
});
...
roboshop.post("/setOrder", async (req, res) => {
  try {
    ...
    const body = parsed.body || {};
    const response = await rds.setOrder(body);
    return sendJson(res, response.statusCode || 200, response.body || {});
  } catch (err) {
    return sendJson(res, 500, { error: "proxy_failed", message: err.message });
  }
...
if (rbkProxyEnabled) {
  ...
  const rbkTargetHost = process.env.RBK_TARGET_HOST || rdsHost;
