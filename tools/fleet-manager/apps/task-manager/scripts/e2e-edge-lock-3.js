...
const stateDir = path.resolve(taskManagerDir, 'state-e2e-edge-lock-3');
...
const dropTargets = new Set(['A1', 'A4']);
...
async function run() {
  ...
  try {
    ...
    await setWorksiteFilled(ports.http, 'PICK-B4', true);

    const tasks = await waitForTasks(ports.http, 3, 15000);
    const robotTargets = {};
    tasks.forEach((task) => {
    ...
    if (robotIds.length < 3) {
      throw new Error('expected tasks for three robots (A1, A4, B4 picks)');
    }

    await waitForRobotsAtPicks(ports.http, robotTargets, 25000);
    await monitorSerializedDrops(ports.http, 30000);
    await waitForCompletedTasks(ports.http, 3, 60000);

    exitCode = 0;
    console.log('E2E ok: edge_lock scheduler with three robots.');
  } finally {
