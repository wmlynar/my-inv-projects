...
const workflowPath =
  process.env.E2E_WORKFLOW_PATH || path.resolve(testDataDir, 'workflow-direction.json5');
const stateDir = path.resolve(taskManagerDir, 'state-e2e-edge-lock-direction');
...
async function waitForResults(port, timeoutMs) {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    const tasks = await fetchTasks(port);
    const completed = tasks.filter((task) => task.status === 'completed');
    const failed = tasks.filter((task) => task.status === 'failed');
    if (completed.length >= 1 && failed.length >= 1) {
      return { completed, failed };
    }
    await delay(300);
  }
  throw new Error('timeout waiting for completed + failed tasks');
}
...
async function run() {
  ...
  try {
    ...
    await waitForResults(ports.http, 45000);

    exitCode = 0;
    console.log('E2E ok: edge_lock scheduler respects directed edges.');
  } finally {
