...
const stateDir = path.resolve(taskManagerDir, 'state-e2e-edge-lock-intersection');
...
async function run() {
  ...
  try {
    ...
    if (robotIds.length < 2) {
      throw new Error('expected tasks for two robots (A and C picks)');
    }

    await waitForRobotsAtPicks(ports.http, robotTargets, 20000);
    await monitorSerializedDrops(ports.http, 20000);
    await waitForCompletedTasks(ports.http, 2, 45000);

    exitCode = 0;
    console.log('E2E ok: edge_lock serialized intersection drops.');
  } finally {
