...
const stateDir = path.resolve(taskManagerDir, 'state-e2e-edge-lock-cancel');
...
async function run() {
  ...
  try {
    ...
    const cancelRes = await httpRequest({
      method: 'POST',
      port: ports.http,
      path: `/api/robots/${encodeURIComponent(dropTask.robotId)}/cancel`
    });
    ...
    exitCode = 0;
    console.log('E2E ok: cancel releases edge_lock reservations.');
  } finally {
