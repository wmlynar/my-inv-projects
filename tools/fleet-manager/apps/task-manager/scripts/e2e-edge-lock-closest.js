...
const stateDir = path.resolve(taskManagerDir, 'state-e2e-edge-lock-closest');
...
async function run() {
  ...
  const manager = spawnProcess('manager', managerPath, {
    ...
    ROBOT_SELECT_STRATEGY: 'closest_idle'
  });
  ...
  try {
    ...
    await setWorksiteFilled(ports.http, 'PICK-B1', true);
    ...
    if (Object.keys(robotTargets).length < 3) {
    ...
    exitCode = 0;
    console.log('E2E ok: closest_idle completes without deadlock.');
  } finally {
