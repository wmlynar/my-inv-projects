...
const stateDir = path.resolve(taskManagerDir, 'state-e2e-edge-lock-stall');
...
const stallTimeoutMs = Number.parseInt(process.env.E2E_STALL_TIMEOUT_MS || '1200', 10);
const pickPointById = {
  'PICK-A1': 'A1',
  'PICK-A4': 'A4'
};
...
async function waitForRobotCompleted(port, robotId, timeoutMs) {
  ...
  while (Date.now() - start < timeoutMs) {
    const tasks = await fetchTasks(port);
    const done = tasks.find((task) => task.robotId === robotId && task.status === 'completed');
    if (done) {
      return done;
    }
    await delay(300);
  }
  throw new Error(`timeout waiting for ${robotId} completion`);
}
...
async function run() {
  ...
  const manager = spawnProcess('manager', managerPath, {
    ...
    ROBOT_STALL_TIMEOUT_MS: String(stallTimeoutMs)
  });
  ...
  try {
    ...
    const stopRes = await httpRequest({
      method: 'POST',
      port: ports.http,
      path: `/api/robots/${encodeURIComponent(dropTask.robotId)}/stop`
    });
    if (stopRes.status !== 200) {
      throw new Error('failed to stop robot');
    }

    await waitForTaskStatus(ports.http, dropTask.id, 'failed', stallTimeoutMs + 8000);
    const remainingRobot = robotIds.find((id) => id !== dropTask.robotId);
    await waitForRobotCompleted(ports.http, remainingRobot, 45000);

    exitCode = 0;
    console.log('E2E ok: stall timeout releases edge_lock reservations.');
  } finally {
