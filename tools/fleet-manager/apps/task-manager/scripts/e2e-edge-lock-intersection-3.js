...
const workflowPath =
  process.env.E2E_WORKFLOW_PATH || path.resolve(testDataDir, 'workflow-intersection-3.json5');
const stateDir = path.resolve(taskManagerDir, 'state-e2e-edge-lock-intersection-3');
...
const dropTargets = new Set(['A', 'B', 'D']);
...
async function run() {
  ...
  try {
    ...
    await setWorksiteFilled(ports.http, 'PICK-D', true);
    ...
    exitCode = 0;
    console.log('E2E ok: edge_lock intersection serialized with three robots.');
  } finally {
