...
const stateDir = path.resolve(taskManagerDir, 'state-e2e-edge-lock-blocked-timeout');
...
const holdMs = Math.max(lockTimeoutMs * 3, 2000);
const pickPointById = {
  'PICK-A1': 'A1',
  'PICK-A4': 'A4'
};
...
async function setSimBlocked(port, blocked) {
  const res = await httpRequest({
    method: 'POST',
    port,
    path: '/sim/blocked',
  ...
  if (res.status !== 200) {
    throw new Error(`failed to set blocked=${blocked}`);
  }
}
...
async function run() {
  ...
  const sim1 = spawnProcess('sim-1', simPath, {
    ...
    TICK_MS: '100',
    SPEED_M_S: speed,
    HTTP_PORTS: String(simHttpPorts['RB-01'])
  });
  ...
  const sim2 = spawnProcess('sim-2', simPath, {
    ...
    TICK_MS: '100',
    SPEED_M_S: speed,
    HTTP_PORTS: String(simHttpPorts['RB-02'])
  });
  ...
  try {
    ...
    if (Object.keys(robotTargets).length < 2) {
    ...
    const simPort = simHttpPorts[dropTask.robotId];
    if (!simPort) {
      throw new Error(`missing sim HTTP port for robot ${dropTask.robotId}`);
    }

    await setSimBlocked(simPort, true);
    await assertDropSerialized(ports.http, holdMs);

    await setSimBlocked(simPort, false);
    await waitForCompletedTasks(ports.http, 2, 45000);

    exitCode = 0;
    console.log('E2E ok: blocked robot holds locks beyond timeout.');
  } finally {
