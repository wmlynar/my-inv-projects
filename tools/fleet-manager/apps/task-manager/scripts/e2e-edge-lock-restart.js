...
const stateDir = path.resolve(taskManagerDir, 'state-e2e-edge-lock-restart');
...
function assertSameTaskIds(actual, expected) {
  if (actual.size !== expected.size) {
    throw new Error(`expected ${expected.size} tasks after restart, got ${actual.size}`);
  }
  for (const id of expected) {
    if (!actual.has(id)) {
      throw new Error(`expected task ${id} to persist after restart`);
    }
  }
}
...
async function run() {
  ...
  try {
    ...
    const taskIdsBefore = getTaskIdSet(await fetchTasks(ports.http));
    ...
    await waitForHttpReady(`http://127.0.0.1:${ports.http}/health`, 15000);
    assertSameTaskIds(getTaskIdSet(await fetchTasks(ports.http)), taskIdsBefore);
    await waitForCompletedTasks(ports.http, 2, 45000);

    exitCode = 0;
    console.log('E2E ok: restart resumes edge_lock tasks.');
  } finally {
    manager.kill('SIGTERM');
    sim1.kill('SIGTERM');
    sim2.kill('SIGTERM');
