...
const stateDir = path.resolve(taskManagerDir, 'state-e2e-edge-lock-fallback-last-station');
...
async function run() {
  ...
  const simEnv = {
    ...
    STATUS_HIDE_POSE: '1',
    STATUS_LAST_STATION_IS_CURRENT: '1'
  };
  ...
  try {
    ...
    exitCode = 0;
    console.log('E2E ok: start-node fallback via last_station keeps edge_lock serialization.');
  } finally {
