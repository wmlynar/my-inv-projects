...
const workflowPath =
  process.env.E2E_WORKFLOW_PATH || path.resolve(testDataDir, 'workflow-same-direction.json5');
const stateDir = path.resolve(taskManagerDir, 'state-e2e-edge-lock-no-same-direction');
...
async function waitForSerializedSameDirection(port, timeoutMs) {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    const tasks = await fetchTasks(port);
    const inProgress = tasks.filter((task) => task.status === 'in_progress');
    const toDrop = inProgress.filter((task) => task.phase === 'to_drop');
    const toPick = inProgress.filter((task) => task.phase === 'to_pick');
    if (toDrop.length === 1 && toPick.length >= 1) {
      return;
    }
    await delay(200);
  }
  throw new Error('timeout waiting for same-direction block');
}
...
async function assertDropSerialized(port, durationMs) {
  ...
  while (Date.now() - start < durationMs) {
    ...
    const toDrop = inProgress.filter((task) => task.phase === 'to_drop');
    if (toDrop.length > 1) {
      throw new Error('expected same-direction dispatch to remain serialized');
    }
    await delay(200);
  }
}
...
async function run() {
  ...
  try {
    ...
    await waitForSerializedSameDirection(ports.http, 12000);
    await assertDropSerialized(ports.http, 2000);
    await waitForCompletedTasks(ports.http, 2, 45000);

    exitCode = 0;
    console.log('E2E ok: allowSameDirection=false serializes same-direction drops.');
  } finally {
