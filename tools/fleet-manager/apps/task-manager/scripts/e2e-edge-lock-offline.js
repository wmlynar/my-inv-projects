...
const stateDir = path.resolve(taskManagerDir, 'state-e2e-edge-lock-offline');
...
const offlineTimeoutMs = Number.parseInt(process.env.E2E_OFFLINE_TIMEOUT_MS || '1200', 10);
const pickPointById = {
  'PICK-A1': 'A1',
  'PICK-A4': 'A4'
};
...
async function run() {
  ...
  const manager = spawnProcess('manager', managerPath, {
    ...
    ROBOT_OFFLINE_TIMEOUT_MS: String(offlineTimeoutMs)
  });
  ...
  try {
    ...
    if (dropTask.robotId === 'RB-01') {
      sim1.kill('SIGTERM');
    } else {
      sim2.kill('SIGTERM');
    }

    await waitForTaskStatus(ports.http, dropTask.id, 'failed', offlineTimeoutMs + 8000);
    const remainingRobot = robotIds.find((id) => id !== dropTask.robotId);
    await waitForRobotCompleted(ports.http, remainingRobot, 45000);

    exitCode = 0;
    console.log('E2E ok: offline robot releases edge_lock reservations.');
  } finally {
