...
const stateDir = path.resolve(taskManagerDir, 'state-e2e-edge-lock-fallback-invalid-current-scheduler');
...
async function run() {
  ...
  const sim1 = spawnProcess('sim-1', simPath, {
    ...
    TICK_MS: '100',
    SPEED_M_S: speed,
    STATUS_FORCE_CURRENT_STATION: 'INVALID-STATION',
  ...
  try {
    ...
    exitCode = 0;
    console.log('E2E ok: invalid current_station falls back to pose for edge_lock.');
  } finally {
