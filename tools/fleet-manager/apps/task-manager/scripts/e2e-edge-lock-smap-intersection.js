...
const workflowPath =
  process.env.E2E_WORKFLOW_PATH || path.resolve(testDataDir, 'workflow-smap-1-intersection.json5');
const stateDir = path.resolve(taskManagerDir, 'state-e2e-edge-lock-smap-intersection');
...
async function run() {
  ...
  try {
    ...
    exitCode = 0;
    console.log('E2E ok: smap intersection serialized via node locks.');
  } finally {
