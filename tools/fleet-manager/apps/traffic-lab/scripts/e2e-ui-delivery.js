...
const DEADLOCK_HOLD_MS = 12000;
const NO_PROGRESS_MS = 15000;
const TASK_STALL_MS = 12000;
const TASK_BLOCKED_MS = 6000;
const TASK_UNASSIGNED_MS = 6000;
...
const trafficStrategy =
  process.env.E2E_TRAFFIC_STRATEGY || process.env.TRAFFIC_STRATEGY || "simple";
...
function assertNoDeadlock(snapshot, watchMap, now) {
  ...
  robots.forEach((robot) => {
    ...
    const reason = diagnostics.reason || null;
    const isHolding = state === "holding" && !HOLD_IGNORE_REASONS.has(reason);
...
function assertProgress(snapshot, progressState, now, targets) {
  const pickIds = targets?.pickIds || new Set();
  const dropIds = targets?.dropIds || new Set();
  const worksites = snapshot.worksites || [];
  const worksiteById = new Map(worksites.map((site) => [site.id, site]));
  const picksPending = [...pickIds].some((id) => worksiteById.get(id)?.filled);
  const dropsPending = [...dropIds].some((id) => !worksiteById.get(id)?.filled);
  const hasActiveTasks = (snapshot.tasks || []).some((task) =>
    ACTIVE_TASK_STATUSES.has(task.status)
  );
  if (!picksPending && !dropsPending && !hasActiveTasks) {
...
async function run() {
  ...
  try {
    ...
    assert.ok(robots.length > 1, "Expected at least two robots for deadlock detection");
    ...
    const targetCount = Math.min(2, sortedPicks.length, sortedDrops.length);
    const selectTargets = (list) => {
      const preferred = [1, 2].map((idx) => list[idx]).filter(Boolean);
      if (preferred.length >= targetCount) return preferred.slice(0, targetCount);
      return list.slice(0, targetCount);
    };
    const targetPicks = selectTargets(sortedPicks);
    const targetDrops = selectTargets(sortedDrops);
    assert.ok(targetPicks.length > 0, "Expected a target pick");
    assert.ok(targetDrops.length > 0, "Expected a target drop");

    for (const site of sortedDrops) {
      const isTarget = targetDrops.some((target) => target.id === site.id);
    ...
    for (const site of sortedPicks) {
      const isTarget = targetPicks.some((target) => target.id === site.id);
      await postJson(baseUrl, `/api/fleet/worksites/${encodeURIComponent(site.id)}`, {
        filled: isTarget,
        blocked: false
      });
    ...
    const pickIds = new Set(targetPicks.map((pick) => pick.id));
    const dropIds = new Set(targetDrops.map((drop) => drop.id));
    ...
    const finished = await waitFor(
      async () => {
        const next = await fetchJson(baseUrl, "/api/fleet/status?worksites=1");
        const now = Date.now();
        const activeRobots = buildActiveRobotSet(next.tasks);
        assertNoDeadlock(next, deadlockWatch, now);
        assertTaskProgress(next, taskWatch, now);
        assertProgress(next, progressWatch, now, { pickIds, dropIds });
        assertNoYield(next);
        assertNoBlocks(next);
        assertNoFlicker(next, flickerWatch, now);
        ...
        const nextWorksites = next.worksites || [];
        const picksEmpty = [...pickIds].every(
          (id) => !nextWorksites.find((site) => site.id === id)?.filled
        );
        const dropsFilled = [...dropIds].every(
          (id) => nextWorksites.find((site) => site.id === id)?.filled
        );
        (next.tasks || []).forEach((task) => {
          if (pickIds.has(task.pickId)) {
            observedTasks.add(task.pickId);
          }
        });
        return picksEmpty && dropsFilled;
      },
      { timeoutMs: 60000, intervalMs: 350 }
    );

    assert.ok(finished, "Expected UI flow to complete pick->drop deliveries");
    assert.equal(
      observedTasks.size,
      pickIds.size,
      "Expected each pick site to create a delivery task"
    );
    console.log("E2E UI delivery ok: UI + backend flow delivered all pick sites.");
  } finally {
    if (!serverExited) {
      server.kill("SIGTERM");
    }
