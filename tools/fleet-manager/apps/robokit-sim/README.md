...
```bash
SIM_PROFILE=local npm --prefix /home/inovatica/seal/monorepo/tools/fleet-manager/apps/robokit-sim start
```
...
- Defaults live in `apps/robokit-sim/rds_params.json` and `apps/robokit-sim/rds_device_types.json`.
- Override with `RDS_PARAMS_PATH` and `RDS_DEVICE_TYPES_PATH`.

Roboshop script simulator:
- `apps/robokit-sim/syspy-js` provides a JS `SimModule` wired into the simulator state.
