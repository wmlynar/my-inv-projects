...
class SimModule {
  ...
  fork() {
    ...
    return {
      ...
      forward_in_place: robot.fork ? robot.fork.forwardInPlace : false,
  ...
  release() {
    const robot = this.robot;
    if (robot) {
      robot.lockInfo = { locked: false, nick_name: '' };
    }
    return 0;
  }
  ...
  requireByNickName(nick_name) {
    const robot = this.robot;
    if (robot) {
      robot.lockInfo = { locked: true, nick_name: String(nick_name || '') };
    }
    return 0;
  }
  ...
  require() {
    const robot = this.robot;
    if (robot) {
      robot.lockInfo = { locked: true, nick_name: robot.id || '' };
    }
    return 0;
  }
