...
@Component
public class FeiShuErrorReportingStrategy
implements ErrorReportHandle {
    private static final Logger log = LoggerFactory.getLogger(FeiShuErrorReportingStrategy.class);
    ...
    public FeiShuErrorReportingStrategy(LocaleMessageUtil localeMessageUtil, CoreAlarmsMapper coreAlarmsMapper, RobotAlarmsMapper robotAlarmsMapper, LocaleConfig localeConfig, TaskAlarmsMapper taskAlarmsMapper, GetMessageService getMessageService) {
    ...
    public boolean report(CommonConfig config) {
        ...
        HashMap map;
        String content;
        String webhookFeiShu = config.getSendAlarms().getApproach().getFsRobot().getWebhook();
        ...
        if (CollectionUtils.isNotEmpty((Collection)BusinessErrorReporting.coreMsg)) {
            try {
                ...
                if (StringUtils.isNotEmpty((CharSequence)content2)) {
                    ...
                    contentMap2.put("text", vxMessage2);
                    map2.put("content", contentMap2);
                    OkHttpUtil.postJson((String)webhookFeiShu, (String)JSONObject.toJSONString((Object)map2));
                    BusinessErrorReporting.coreMsg.forEach(it -> it.setIsOk(Integer.valueOf(1)));
        ...
        if (CollectionUtils.isNotEmpty((Collection)BusinessErrorReporting.rbkMsg)) {
            try {
                ...
                if (StringUtils.isNotEmpty((CharSequence)content)) {
                    ...
                    contentMap.put("text", vxMessage);
                    map.put("content", contentMap);
                    OkHttpUtil.postJson((String)webhookFeiShu, (String)JSONObject.toJSONString((Object)map));
                    BusinessErrorReporting.rbkMsg.forEach(it -> it.setIsOk(Integer.valueOf(1)));
