 ...
 *  com.seer.rds.service.wind.taskBp.ModbusBatchReadSingleInputBp
 *  com.seer.rds.vo.wind.ModbusBatchReadHoldingRegisterBpField
 *  com.seer.rds.vo.wind.ModbusBatchReadSingleInputBpField
...
import com.seer.rds.vo.wind.ModbusBatchReadSingleInputBpField;
...
@Component(value="ModbusBatchReadSingleInputBp")
@Scope(value="prototype")
public class ModbusBatchReadSingleInputBp
extends AbstractBp {
    private static final Logger log = LoggerFactory.getLogger(ModbusBatchReadSingleInputBp.class);
    @Autowired
    ...
    protected void getInputParamsAndExecute(AbstratRootBp rootBp) throws Exception {
        boolean[] modbusValue;
        this.ipModbusHost = String.valueOf(rootBp.getInputParamValue(this.taskId, this.inputParams, ModbusBatchReadSingleInputBpField.ipModbusHost));
        this.ipModbusPort = Integer.valueOf(rootBp.getInputParamValue(this.taskId, this.inputParams, ModbusBatchReadSingleInputBpField.ipModbusPort).toString());
        this.ipRegisterOffset = Integer.valueOf(rootBp.getInputParamValue(this.taskId, this.inputParams, ModbusBatchReadSingleInputBpField.ipRegisterOffset).toString());
        this.ipSlaveId = Integer.valueOf(rootBp.getInputParamValue(this.taskId, this.inputParams, ModbusBatchReadSingleInputBpField.ipSlaveId).toString());
        this.ipLength = Integer.valueOf(rootBp.getInputParamValue(this.taskId, this.inputParams, ModbusBatchReadSingleInputBpField.ipLength).toString());
        boolean modbusException = false;
        int socketExceptionCount = 0;
        while (true) {
            try {
                WindTaskStatus.monitorTaskEndErrorAndTaskStop((AbstractBp)this);
                modbusValue = Modbus4jUtils.batchReadInput((String)this.ipModbusHost, (int)this.ipModbusPort, (int)this.ipSlaveId, (int)this.ipRegisterOffset, (int)this.ipLength, (String)"ModbusBatchReadSingleCoilBp");
            }
            catch (Exception e) {
                ...
                log.error("ModbusBatchReadSingleInputBp [{},{}] {}]", new Object[]{this.ipModbusHost, this.ipModbusPort, e});
                this.saveLogError(e.getMessage());
                if (!modbusException) {
                    this.windService.saveLog(TaskLogLevelEnum.error.getLevel(), "[ModbusBatchReadSingleInputBp]@{wind.bp.retry}:" + e.getMessage(), this.taskRecord.getProjectId(), this.taskId, this.taskRecord.getId(), this.blockVo.getBlockId());
                    ...
                    this.windService.updateTaskRecordEndedReason(this.taskRecord.getId(), "[ModbusBatchReadSingleInputBp]modbus @{wind.bp.retry}");
                }
        ...
        modbusValue = Arrays.copyOf(modbusValue, (int)this.ipLength);
        ...
        log.info("ModbusBatchReadSingleInputBp " + message);
    ...
    protected void setBlockInputParamsValue(AbstratRootBp rootBp) throws Exception {
        ModbusBatchReadSingleInputBp bpData = new ModbusBatchReadSingleInputBp();
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof ModbusBatchReadSingleInputBp)) {
            return false;
        }
        ModbusBatchReadSingleInputBp other = (ModbusBatchReadSingleInputBp)o;
    ...
    protected boolean canEqual(Object other) {
        return other instanceof ModbusBatchReadSingleInputBp;
    }
    ...
    public String toString() {
        return "ModbusBatchReadSingleInputBp(windService=" + this.getWindService() + ", windTaskRecordMapper=" + this.getWindTaskRecordMapper() + ", ipModbusHost=" + this.getIpModbusHost() + ", ipModbusPort=" + this.getIpModbusPort() + ", ipRegisterOffset=" + this.getIpRegisterOffset() + ", ipSlaveId=" + this.getIpSlaveId() + ", ipLength=" + this.getIpLength() + ")";
    }
}
