...
async function runRelease({ releaseDir, buildId, runTimeoutMs }) {
  ...
  const readyFile = port === null ? makeReadyFile("protection") : null;
...
async function main() {
  ...
  const buildTimeoutMs = resolveE2ETimeout("SEAL_PROTECTION_E2E_BUILD_TIMEOUT_MS", 180000);
  const runTimeoutMs = resolveE2ERunTimeout("SEAL_PROTECTION_E2E_RUN_TIMEOUT_MS", 15000);
  const testTimeoutMs = resolveE2ETimeout("SEAL_PROTECTION_E2E_TIMEOUT_MS", 240000);
