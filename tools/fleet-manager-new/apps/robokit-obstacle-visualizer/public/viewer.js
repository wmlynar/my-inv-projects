(() => {
  ...
  function createCursorLayer() {
    ...
    return {
      ...
      render({ svg, state: layerState, helpers, visible }) {
        ...
        const direction = invertY ? -yaw : yaw;
  ...
  function projectPoint(point) {
    ...
    return {
      ...
      y: Number.isFinite(y) ? (invertY ? -y : y) : 0
    };
  }
  ...
  async function loadInitialData() {
    try {
      ...
      invertY = session?.viewer?.invertY === false ? false : true;
  ...
  function loadManualMap(file, key) {
    ...
    reader.onload = () => {
      try {
        const data = parseJson5(String(reader.result));
