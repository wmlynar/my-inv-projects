...
const DEFAULT_PROFILES_B64 = path.resolve(__dirname, 'data', 'http_getProfiles.b64');
const DEFAULT_ROBOTS_STATUS_B64 = path.resolve(__dirname, 'data', 'http_robotsStatus.b64');
...
function loadBase64Body(filePath) {
  if (!filePath) {
    return null;
  }
  try {
...
function loadJsonFromBase64(filePath) {
  const buffer = loadBase64Body(filePath);
  if (!buffer) {
    return null;
  }
  try {
...
function updateRobotsStatusPayload(payload) {
  if (!payload || typeof payload !== 'object') {
    return payload;
  }
  payload.create_on = nowIsoNoMs();
  return payload;
}
...
function sendRawJson(res, statusCode, buffer, extraHeaders = {}) {
...
async function handleRequest(req, res, options) {
  ...
  if (pathname === '/getProfiles' && req.method === 'POST') {
    if (options.profilesBody) {
      sendRawJson(res, 200, options.profilesBody, {
        Connection: 'keep-alive',
        'Keep-Alive': 'timeout=30, max=255'
      });
    } else {
      sendJson(res, 200, { ok: true });
    }
  ...
  if (pathname === '/robotsStatus' && req.method === 'GET') {
    if (options.robotsStatusTemplate) {
      const payload = updateRobotsStatusPayload(cloneJson(options.robotsStatusTemplate));
      sendJson(res, 200, payload, {
        uuid: createUuid(),
        Connection: 'keep-alive',
        'Keep-Alive': 'timeout=30, max=255'
      });
    } else if (options.robotsStatusBody) {
      sendRawJson(res, 200, options.robotsStatusBody, {
        uuid: createUuid(),
        Connection: 'keep-alive',
        'Keep-Alive': 'timeout=30, max=255'
      });
...
function startHttpStub(options = {}) {
  ...
  const profilesBody = options.profilesBody || loadBase64Body(process.env.HTTP_PROFILES_B64 || DEFAULT_PROFILES_B64);
  const robotsStatusBody =
    options.robotsStatusBody || loadBase64Body(process.env.HTTP_ROBOTS_STATUS_B64 || DEFAULT_ROBOTS_STATUS_B64);
  const robotsStatusTemplate =
    options.robotsStatusTemplate ||
    loadJsonFromBase64(process.env.HTTP_ROBOTS_STATUS_B64 || DEFAULT_ROBOTS_STATUS_B64);
  const serverOptions = {
    maxBodyLength,
    sceneZipPath,
    coreParams,
