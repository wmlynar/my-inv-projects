...
function createTempMap() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'robokit-approach-near-'));
  const mapPath = path.join(dir, 'approach-near-map.json');
  const map = {
    meta: {
      mapName: 'approach-near-map',
      mapType: '2D-Map',
      version: '1.0.0'
    },
    nodes: [
...
async function run() {
  ...
  try {
    ...
    assert(unfinished.length > 0, 'expected unfinished_path when start is near graph');

    let stayedNonEmpty = true;
    const deadline = Date.now() + 4000;
    while (Date.now() < deadline) {
      await wait(200);
      const t = await request(ports.STATE, API.robot_status_task_req, {});
      const list = Array.isArray(t.unfinished_path) ? t.unfinished_path : [];
      if (list.length === 0) {
        stayedNonEmpty = false;
        break;
      }
    }
    assert(stayedNonEmpty, 'unfinished_path should not be empty when approach is skipped');

    console.log('e2e_approach_skip_when_near.test ok');
  } catch (err) {
    console.error('e2e_approach_skip_when_near.test failed');
    if (logs) {
