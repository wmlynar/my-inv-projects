...
test('mvp0 runtime executes pick->drop->park flow for single robot', () => {
  ...
  assert.equal(decision.commands[0].payload.operation, 'ForkLoad');
  ...
  assert.equal(decision.commands[0].payload.operation, 'ForkUnload');

  runtime.upsertRobotStatus('R1', { nodeId: 'AP_DROP' }, now + 2000);
...
test('mvp0 runtime skips park when pending tasks exist', () => {
  ...
  runtime.upsertRobotStatus('R1', { nodeId: 'AP_DROP_1' }, now + 2000);
  decision = runtime.tick({ nowMs: now + 2000 });
