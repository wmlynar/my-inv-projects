...
- próbkowanie po `edgeS` (np. co `cellLen/2`),
...
\[
d\_{stop}(v) = v \cdot t\_{reaction} + rac{v^2}{2\cdot brake} + stopExtra
\]
...
#### 11.8.2 Składnik „standoff‑limited” (follow distance)

`stopStandoff` jest **polityką ruchu**: nawet jeśli 2D obwiednie by się nie zderzyły, robot nie powinien dojechać „na zderzak”.

Dla każdej pary robotów w **tym samym `corridorId` i tym samym kierunku** wyznaczamy limit postępu robota „z tyłu”:

\[
hold\_{standoff} = leaderRouteS - leaderTrailExtent - followerLeadExtent - stopStandoff - standoffExtra
\]

gdzie:
- `leaderTrailExtent` i `followerLeadExtent` zależą od tego, czy robot w danej sekcji jedzie przodem czy tyłem:  
  - **gdy robot jedzie przodem:** `leadExtent = head + safetyFront`, `trailExtent = tail + safetyRear`  
  - **gdy robot jedzie tyłem:** `leadExtent = tail + safetyRear`, `trailExtent = head + safetyFront`  
- `standoffExtra` zawiera marginesy niepewności (`poseMargin`, `trackingMargin`) oraz opcjonalny dodatkowy bufor na zakrętach.

**[MVP] MUST:** jeśli robot „przód” ma stan `UNKNOWN/STALE/OFF_ROUTE`, traktujemy go jak przeszkodę i `hold_standoff` jest liczony konserwatywnie (lub spada do „0”, wymuszając stop).
...
Jeśli na odcinku trasy brakuje `LocationMark` dostatecznie blisko `holdPointRouteS`, to `RobotGateway` musi egzekwować `stopLineRouteS` niezależnie od tego, że target mark jest dalej (np. przez `pause`/`stop` w odpowiednim momencie). To jest część fail‑safe MVP.
