# Sanden Etap 1 - Task odbioru wyrobu gotowego z A1 P3
...
- Czyszczenie requestu `A1_P3_REMOVE`: Etap 0.1 i Etap 1 czyscimy po ForkLoad.
...
Zadanie `sanden_remove_full_p3` odbiera nosnik z wyrobem gotowym
z pola P3 linii A1 i dowozi go do bufora odbiorow w magazynie.

Mapowanie na dokument Etap 1 (`sanden_etap_1.md`):
- P3 (A1) odpowiada polu `A1_OUT` / `PROD_A1_OUT`,
- strumien S3: `PROD_A1_OUT -> MAG_OUT_COLLECT_TMP`.
...
- `klienci/sanden/tasks/sanden_remove_empty_p1_spec.md` (konwencje)
...
- Worksite request: `A1_P3_REMOVE`
- Znaczenie: `filled = 1` oznacza aktywne zgloszenie odbioru wyrobu gotowego z P3.
- `A1_P3_REMOVE` sluzy rowniez jako punkt pobrania (source) na mapie.

Uwaga:
- nie ma osobnego worksite zajetosci P3; request jest jedynym sygnalem.
- `A1_P3_REMOVE` musi istniec jako worksite (dla `SetSiteEmptyBp`),
  i jako label na mapie (dla `ForkLoad`).

### 2.2 Bufor odbiorow w magazynie (drop)

Docelowy bufor odbiorow to grupa `PUT_FULL_A1`
(wspolny bufor odbiorow, ten sam co dla P1):
...
Opcjonalnie:
- Pre-pick: punkt przy P3 (np. `LM_P3`),
...
Etap 0.1 (domyslne):
- `PICK_KEY_ROUTE = "LM_P3"`
- `PRIORITY = "0"` (opcjonalnie)
- `PRE_PICK_SITE = ""`
- `PRE_DROP_SITE = ""`
- `DROP_GROUP = "PUT_FULL_A1"`
...
Task manager uruchamia `sanden_remove_full_p3` tylko gdy:
- `A1_P3_REMOVE` jest `filled = 1`, `disabled = 0`, `locked = 0`,
- istnieje przynajmniej jeden wolny worksite w grupie `PUT_FULL_A1`
  z `filled = 0`, `locked = 0`, `disabled = 0`,
- nie ma aktywnego taska `sanden_remove_full_p3` (targetActiveTasks = 1).
...
Uwaga (Etap 0.1):
- cofniecie `A1_P3_REMOVE` po starcie taska jest ignorowane (task kontynuuje),
...
## 4. Wybor miejsca drop (bufor odbiorow)
...
```json
{ "from": "A1_P3_REMOVE", "to": "PUT_FULL_A1_1" }
```
...
Wspoldzielenie bufora z P1 (Etap 1):
...
- istnieje worksite `A1_P3_REMOVE` i jest enabled,
- `A1_P3_REMOVE` istnieje na mapie (label dla ForkLoad),
...
Opcjonalnie:
- ustawienie `content`/tagow na dropie (np. "FULL_P3") - tylko jesli wymagane przez magazyn.
...
- Task (.task): `klienci/sanden/tasks/sanden_remove_full_p3.task`
- Spec: `klienci/sanden/tasks/sanden_remove_full_p3_spec.md`
- Skrypty (jesli beda potrzebne): `klienci/sanden/scripts/*.js`

## 11. Testy akceptacyjne (Etap 0.1)

- Przy aktywnym `A1_P3_REMOVE` i wolnym `PUT_FULL_A1_*` task startuje.
- Po ForkLoad `A1_P3_REMOVE` jest czyszczony.
...
- Potwierdz mapowanie P3 <-> `A1_OUT` / `PROD_A1_OUT` w RDS.
- Etap 1: czy drop ma byc lockowany (`GET_IDLE_LOCK=true`) i czy wymagamy
  jawnego unlock po sukcesie.
- Czy potrzebujemy wariantu A2 (A2_P3_REMOVE -> PUT_FULL_A1) w Etapie 1.
