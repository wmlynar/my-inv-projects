...
- Wdrożenie (systemd / manual):
  - Uruchomienie: `node modbus-sync-worksites.js` (katalog projektu).
  - Systemd: `WorkingDirectory` ustawione na katalog projektu, `ExecStart=node modbus-sync-worksites.js`.

- Skrypty operatorskie (legacy):
  - Stare skrypty przeniesione do `old/scripts/` – nieużywane w produkcji.
...
- Repozytorium:
  ...
  - Commitowane: kod (`*.js`), skrypty bash (legacy), `package.json`, `package-lock.json`, `.gitignore`, dokumentacja (`requirements.md`).
...
- Konfiguracja:
  - wszystkie istotne parametry (hosty, timeouty, interwały) zebrane w jednym miejscu w kodzie,
...
- Aplikacje UI (frontend):
  ...
  - Backend wystawia `/api/status` z `buildId` (np. z `version.json` lub `package.json`).
...
- Testy / narzędzia:
  ...
  - narzędzia operatorskie: logi, restart, status, test.

- Repozytorium:
  - kod w gicie,
  - powtarzalny sposób wdrożenia (release + deploy),
  - `.env` nie jest wersjonowane.
